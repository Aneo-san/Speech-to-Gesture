...
def main():
    ckpt_path = Path(__file__).resolve().parent.parent / "models" / "lstmp.pt"
    ...
    def update(_):
        ...
        try:
            ...
            for k, (a, b) in enumerate(edges):
                pa = pos[id(a)]
                pb = pos[id(b)]
                xa, ya, za = float(pa[0]), float(pa[1]), float(pa[2])
                xb, yb, zb = float(pb[0]), float(pb[1]), float(pb[2])
                # set 3D data
                lines[k].set_data([xa, xb], [ya, yb])
                try:
                    lines[k].set_3d_properties([za, zb])
                except Exception:
                    # fallback for older mpl
                    lines[k].set_zdata([za, zb])
            ...
            ax.set_title(f"Realtime gesture (3D) (hop={chunk_sec:.1f}s, hubert_win={hubert_win_sec:.1f}s, EMA={ema_alpha:.2f}) | draw_fps≈{draw_fps:.1f}")
            return lines
        except Exception:
